...
local PatchOptionsPtr = (mem.dll[AppPath.."mm"..mmver.."patch"] or {}).GetOptions
...
function structs.f.GameStructure(define)
	...
	if mmver == 6 then
		define
		[0x970C7C].array(31).EditPChar  'SkillNames'
		[0x56F394].array(31).EditPChar  'SkillDescriptions'
		[0x56B6C0].array(18).EditPChar  'ClassDescriptions'
		[0x970BEC].array(18).EditPChar  'ClassNames'
	else
		define
		[mmv(nil, 0xAE3150, 0xBB3060)].array(37).EditPChar  'SkillNames'
		[mmv(nil, 0x5C88F0, 0x5E4CB0)].array(37).EditPChar  'SkillDescriptions'
	...
	if mmver == 6 then
		define[mmv(0x55F628, nil, nil)].array{100, lenA = i4, lenP = mmv(0x55F628, nil, nil) + 5600}.struct(structs.MapStatsItem)  'MapStats'
	else
	...
	[mmv(0x908D08, 0xACCE64, 0xB20EBC)].i8  'Time'
	 .Info "256*(minutes since 00 AM 1 January 1165/1168/1172)"
	...
	[mmv(0x908E30, 0xACD6B4, 0xB21728)].i4  'TurnBased'
	[mmv(0x4C7DF4, 0x4F86DC, 0x509C9C)].i4  'TurnBasedPhase'  -- 1 = monsters move, 2 = combat, 3 = party walking
	...
	if mmver == 6 then
		define[mmv(0x9CF5C0, nil, nil)].i4  'MasterVolume'
	end
	if mmver > 6 then
	...
	-- MM6:
	-- 4BE3B8 Event2DMapMoveX
	-- 4BE3D0 Event2DMapMoveY
	-- 4BE3E8 Event2DMapMoveZ
	-- 4BE400 Event2DMapMoveDirection
	...
	if mmver > 6 then
		define[mmv(nil, 0x5C8B40, 0x5E4F08)].array(89).array(89).u1  'HostileTxt'  -- HostileTxt[mon1][mon2] is attitude of mon1 towards mon2. mon2 = 0 is party.
		 .Info "HostileTxt[mon1][mon2] is attitude of mon1 to mon2. mon2 = 0 is party."
	end
	define
	...
	[mmv(0x5E2178, 0x5FEFCC, 0x61C534) + 4].parray{lenA = i4, lenP = mmv(0x5E2178, 0x5FEFCC, 0x61C534)}.struct(structs.MonListItem)  'MonListBin'
	...
	if mmver == 6 then
		define.func{name = "ShowMovie", p = mmv(0x4A59A0, nil, nil), cc = 2, must = 1; "", 0, 1}
	elseif mmver == 7 then
	...
	.func{name = "SummonMonster", p = mmv(0x4A35F0, 0x4BBEC4, 0x4BA076), cc = 2, must = 4}
	 .Info{Sig = "Id, X, Y, Z"}
	if mmver > 6 then
		define.func{name = "IsMonsterOfKind", p = mmv(nil, 0x438BCE, 0x436542), cc = 2, must = 2}
	...
	function define.f.LoadDataFileFromLod(name, UseMalloc)
		return mem.call(mmv(0x40C1A0, 0x410897, 0x411C9B), 1, mmv(0x4CB6D0, 0x6BE8D8, 0x6FB828), name, UseMalloc)
	end
	define.Info{Sig = "Name, UseMalloc"}
	function define.f.LoadTextFileFromLod(name)
		local p = Game.LoadDataFileFromLod(name)
		if p ~= 0 then
...
if mmver == 6 then
	...
	function structs.f.GameClassKinds(define)
		...
		[0x4C2694].array(6).array(31).u1  'StartingSkills'
		 .Info "0 - not available,  1 - given on start,   2 - can choose on start,    3 - can learn"
	end
...
elseif mmver == 7 then
	...
	function structs.f.GameClasses(define)
		...
		[0x4ED818].array(36).array(37).u1  'Skills'
		 .Info "0 - not available,  1 - Basic,  2 - Expert,  3 - Master, 4 - GM"
	...
	function structs.f.GameClassKinds(define)
		...
		[0x4ED6C8].array(9).array(37).u1  'StartingSkills'
		 .Info "0 - can't choose, 1 - can choose on start, 2 - given on start"
	end
...
elseif mmver == 8 then
	...
	function structs.f.GameClasses(define)
		...
		[internal.SPStatKinds].array(16).u1  'SPStats'  -- 0 - no SP, 1 - Intellect, 2 - Personality, 3 - both
	end
...
function structs.f.GameParty(define)
	...
	.skip(4)
	.i4  'StableZ'
	 .Info "Z changes up and down during flying, StableZ stays the same"
	.i4  'LastStandFacet'
	.skip(8)
	.i4  'FallStartZ'
	.b4  'Flying'
	...
	.func{name = "AddGold", p = mmv(0x41EDE0, 0x420BAE, 0x420131), cc = 2, must = 1; 0, 0}
	 ...
	 "2 = [MM7, MM8] take all and don't show message, just clear status message\n"..
	 "3 = [MM7, MM8] take all and don't change status message"}
	if mmver < 8 then
...
function structs.f.Player(define)
	...
	[mmv(0x105C, 0x157C, 0x1810)].array(126).i4  'Inventory'
	 .Info "positive: item number,  negative: main inventory slot number"
	if mmver == 6 then
		define
...
function structs.f.PatchOptions(define)
	...
	int  'QuickLoadKey'
	int  'AutorunKey'
	uint  'HDWTRCount'  Info "(unused in MM6)"
	uint  'HDWTRDelay'  Info "(unused in MM6)"
	int  'HorsemanSpeakTime'
	int  'BoatmanSpeakTime'
	single  'PaletteSMul'  Info "(unused in MM6)"
	single  'PaletteVMul'  Info "(unused in MM6)"
	bool  'NoBitmapsHwl'  Info "(unused in MM6)"
	bool  'PlayMP3'
	int  'MusicLoopsCount'
	bool  'HardenArtifacts'  Info "(unused in MM6)"
	bool  'ProgressiveDaggerTrippleDamage'
	bool  'FixChests'
	bool  'DataFiles'
	bool  'FixDualWeaponsRecovery'  Info "(MM6 only)"
	int  'IncreaseRecoveryRateStrength'  Info "(MM6 only)"
	int  'BlasterRecovery'  Info "(unused in MM8)"
	bool  'FixSkyBitmap'  Info "(MM8 only)"
	bool  'NoCD'
	...
	function define.f.Present(name)
	 ...
	 define.Info "Returns true if the given option is supported by patch version being used"
end
