...
if mmver > 6 then
	...
	mem.hookfunction(mm78(0x439E16, 0x4378CD), 2, 1, function(d, def, item, kind, vampiric)
		...
		-- Rod: insert targeted monster and player as well
		t.MonsterIndex, t.Monster = GetMonster(d.esi)
		t.PlayerIndex, t.Player = GetPlayer(i4[d.ebp-8])
		-- Rod
...
mem.hookfunction(mmv(0x421DC0, 0x427522, 0x425951), 0, 3, function(d, def, mon, kind, dmg)
	...
	-- Rod: insert attacker as well
	local aptr = u4[d.ebp-0x8]
	t.ByPlayer	= aptr >= Party.PlayersArray["?ptr"]
	t.Melee		= aptr == u4[d.ebp-0x4]
	if t.ByPlayer then
		t.PlayerIndex, t.Player = GetPlayer(aptr)
	end
	-- Rod
