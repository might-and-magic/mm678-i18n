Editor = Editor or {}
local _KNOWNGLOBALS = FacetRefsLimit
...
local function WriteFacetData(a, t)
	...
	if t.ImportVertex then
		...
		Editor.ImportBitmapU[facetId] = -(ux*vertX + uy*vertY + uz*vertZ) + t.ImportU*b.Width
		Editor.ImportBitmapV[facetId] = -(vx*vertX + vy*vertY + vz*vertZ) + t.ImportV*b.Height
	end
...
local function WriteOutlines()
	...
	-- Rod: limit of map outlines is increased
	local MOLimit = Map.Outlines.Limit
	--
	...
	if #sorted > MOLimit and not Editor.LastError then
		Editor.LastError = ("Too many outlines (%s/" .. MOLimit .. ")"):format(#sorted)
	...
	for i = 1, #t.Items do
		if i <= 140 then
			...
			if type(it) == "number" then
				a.Items[i - 1].Number = it
			else
				table.copy(it, a.Items[i - 1], true)
			end
		else
			t.Items[i] = nil
		end
...
function Editor.UpdateObjectLook(a)
	local id = Game.ItemsTxt[a.Item.Number].SpriteIndex
	a.Type = id
	for i, b in Game.ObjListBin do
		if b.Id == id then
			a.TypeIndex = i
			break
		end
...
local function WriteMonster(a, t)
...
local function WriteMonsters()
	...
	for i, t in ipairs(Editor.Monsters) do
		local a, n = SummonMonster(t.Id, t.X, t.Y, t.Z, true)
		if not a then
...
function Editor.UpdateMonster(n)
	...
	local a = SummonMonster(t.Id, t.X, t.Y, t.Z, true, n)
	WriteMonster(a, t)
end
...
function Editor.UpdateDynamicStuff()
	WriteObjects()
	WriteMonsters()
end
